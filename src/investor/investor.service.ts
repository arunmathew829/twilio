...
export class InvestorService {
  ...
  async update(id: number, updateInvestorDto: UpdateInvestorDto) {
    ...
    if (!investor) return { message: 'User not found' };
    ...
    if (newResetTokenIssuedAt)
      investor.resetTokenIssuedAt = newResetTokenIssuedAt;
  ...
  async removeInvestor(email: string) {
    try {
      ...
      if (investor) {
        await this.investorRepository.delete(investor.id);
        return `Investor ${investor.email} has been deleted!`;
      }
      throw new NotFoundException('Investor does not exist');
    } catch (err) {
      console.log('Error deleting investor', err);
      throw new InternalServerErrorException();
    }
  }
}
